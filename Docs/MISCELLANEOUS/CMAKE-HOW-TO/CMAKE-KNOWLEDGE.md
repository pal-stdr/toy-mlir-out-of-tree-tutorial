...
```
What this `DEPENDS ToyCh2OpsIncGen` is doing here? **Before this library is called in the final compilation process,** cmake will start to generate those `.inc`, `.h.inc` on the fly ahead-of-time 🙂. For your convenience, I'm sharing the final project scaffold.
```sh
└── project-root
    ├── main.cpp
    ├── CMakeLists.txt
